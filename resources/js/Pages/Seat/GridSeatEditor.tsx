...
const GridSeatEditor: React.FC<Props> = ({ initialLayout, onSave, venueId }) => {
  ...
  const initializeGrid = () => {
    const newGrid: GridCell[][] = Array(totalRows).fill(null).map(() =>
      Array(totalColumns).fill(null).map(() => ({ type: 'empty' }))
    );
  ...
  const DimensionControl = () => (
    <div className="mb-6 space-y-4">
      <div className="flex gap-4 items-center">
        <div>
          <label className="block text-sm font-medium text-gray-700">Top Rows</label>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              onClick={() => setDimensions(d => ({ ...d, top: Math.max(0, d.top - 1) }))}
            >
              -
            </Button>
            <span className="w-8 text-center">{dimensions.top}</span>
            <Button
              variant="outline"
              onClick={() => setDimensions(d => ({ ...d, top: d.top + 1 }))}
            >
              +
            </Button>
          </div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Bottom Rows</label>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              onClick={() => setDimensions(d => ({ ...d, bottom: Math.max(1, d.bottom - 1) }))}
            >
              -
            </Button>
            <span className="w-8 text-center">{dimensions.bottom}</span>
            <Button
              variant="outline"
              onClick={() => setDimensions(d => ({ ...d, bottom: d.bottom + 1 }))}
            >
              +
            </Button>
          </div>
      ...
      <div className="flex gap-4 items-center">
        <div>
          <label className="block text-sm font-medium text-gray-700">Left Columns</label>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              onClick={() => setDimensions(d => ({ ...d, left: Math.max(0, d.left - 1) }))}
            >
              -
            </Button>
            <span className="w-8 text-center">{dimensions.left}</span>
            <Button
              variant="outline"
              onClick={() => setDimensions(d => ({ ...d, left: d.left + 1 }))}
            >
              +
            </Button>
          </div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Right Columns</label>
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              onClick={() => setDimensions(d => ({ ...d, right: Math.max(1, d.right - 1) }))}
            >
              -
            </Button>
            <span className="w-8 text-center">{dimensions.right}</span>
            <Button
              variant="outline"
              onClick={() => setDimensions(d => ({ ...d, right: d.right + 1 }))}
            >
              +
            </Button>
          </div>
  ...
  <div className="grid gap-1 w-full">
    {[...grid].reverse().map((row, reversedIndex) => {
      ...
      const rowLabel = getAdjustedRowLabel(reversedIndex, totalRows);
      return (
        <div key={reversedIndex} className="flex gap-1">
          {row.map((cell, colIndex) => (
            <div
              key={colIndex}
              onClick={() => handleCellClick(grid.length - 1 - reversedIndex, colIndex)}
              className={`
                w-8 h-8 
  ...
  <div className="mt-10 w-[50VW] h-10 bg-white border border-gray-200 flex items-center justify-center rounded text-sm self-center">
    Panggung
  </div>
</div>
